 ...
 *
 * Client request (line 186589-186594):
 ...
 *
 *   For each friend:
 *     - lastMsgTime: number (0 if no messages)
 *     - lastReadTime: number (0 if never read)
 *     - msg: string (first 10 chars of last message content)
 *     - userInfo: { _serverId, _oriServerId, _userId, _nickName, _headImage,
 *                   _headEffect, _headBox, _guildName, _level, _vip, _superSkill, _totalPower }
 *       → UserSimpleInfo.deserialize strips _ prefix
 *       → isCommonType only accepts string/number/boolean (arrays like _superSkill are skipped)
 *
 *   Also called on entering game scene (line 236725-236738) with same format.
 */
    ...
    execute: function (data, socket, ctx) {
        return new Promise(function (resolve) {
            try {
                ...
                console.error('  [getMsgList] Error: ' + err.message);
                console.error('  [getMsgList] Stack: ' + err.stack);
