/**
 * ============================================================================
 *  XXTEA Encryption/Decryption — Node.js port
...
function utf8Encode(str) {
    // Simple UTF-8 encoding matching client's Utf8.encode
    var encoded = '';
    for (var i = 0; i < str.length; i++) {
        ...
        if (code < 0x80) {
            ...
            // Surrogate pair (rare for challenge strings)
            var hi = code - 0xD800;
...
function utf8Decode(str) {
    // Simple UTF-8 decoding matching client's Utf8.decode
    var result = '';
    var i = 0;
    while (i < str.length) {
        var code = str.charCodeAt(i);
...
// =============================================
// Client uses charCodeAt for each byte, little-endian:
//   t[n] = charCodeAt(4n) + (charCodeAt(4n+1) << 8) + (charCodeAt(4n+2) << 16) + (charCodeAt(4n+3) << 24)
...
function encrypt(plaintext, key) {
    ...
    var z = data[n - 1];  // s in client (last element)
    var y = data[0];       // l in client (first element)

    while (rounds-- > 0) {
        p = (p + delta) & 0xFFFFFFFF;  // JavaScript doesn't auto-overflow for addition
        var e = (p >>> 2) & 3;
        for (var i = 0; i < n; i++) {
            y = data[(i + 1) % n];
...
function decrypt(ciphertext, key) {
    ...
    result = result.replace(/\0+$/, '');  // Remove trailing nulls
    return utf8Decode(result);
}

// =============================================
// Public API — used by index.js for TEA verification
// =============================================
